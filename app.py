...
# Import the new AI Agent module
from medical_agent import create_agent_chat
...
p1, p2 = st.columns(2)
with p1: run_hybrid = st.button(T("run_hybrid"), type="primary", width="stretch", key="run_hybrid_btn")
with p2: run_clinical_only = st.button(T("run_clinical"), width="stretch", key="run_clinical_btn")
...
# ════════════════════════════════════════════════════════════
# SECTION 6: AI VIRTUAL CARDIOLOGIST (AGENT)
# ════════════════════════════════════════════════════════════

if "last_fusion_result" in st.session_state:
    st.markdown(f'<div class="section-header"><h2>👨‍⚕️ Chat with Dr. AI (Virtual Cardiologist)</h2></div>', unsafe_allow_html=True)
    
    # Initialization
    if "agent_chat" not in st.session_state:
        st.session_state.agent_chat = create_agent_chat()
        st.session_state.chat_history = []
        
        # Inject the context initially so the agent knows the patient\'s state
        if st.session_state.agent_chat:
            context_prompt = f"""
            Hello Dr. AI. I am the patient. Here are my latest test results:
            - Age: {st.session_state.last_clinical_inputs.get('age', 'Unknown')}
            - Blood Pressure (trestbps): {st.session_state.last_clinical_inputs.get('trestbps', 'Unknown')}
            - Cholesterol: {st.session_state.last_clinical_inputs.get('chol', 'Unknown')}
            - Random Forest Risk Probability: {st.session_state.last_clinical_result.get('risk_percent', 'Unknown')}%
            - Final Composite Risk Level: {st.session_state.last_fusion_result.get('risk_level', 'Unknown')}
            
            Please provide a 2 sentence summary telling me what you think of my risk, and let me know I can ask you any questions.
            """
            try:
                # Get the agent to introduce itself
                res = st.session_state.agent_chat.send_message(context_prompt)
                st.session_state.chat_history.append({"role": "assistant", "content": res.text})
            except Exception as e:
                st.session_state.chat_history.append({"role": "assistant", "content": f"Initialization error: {e}"})

    # Render Chat History
    if st.session_state.agent_chat is not None:
        for msg in st.session_state.chat_history:
            with st.chat_message(msg["role"]):
                st.markdown(msg["content"])
        
        # Input for the user to chat
        if user_prompt := st.chat_input("Ask Dr. AI about your results or blood pressure..."):
            ...
            with st.chat_message("assistant"):
                with st.spinner("Dr. AI is thinking..."):
                    try:
                        response = st.session_state.agent_chat.send_message(user_prompt)
                        st.markdown(response.text)
                        st.session_state.chat_history.append({"role": "assistant", "content": response.text})
                    except Exception as e:
                        st.error(f"Error connecting to agent: {e}")
    else:
        st.warning("⚠️ AI Agent is currently unavailable. Please verify the API Key in the `.env` file.")
